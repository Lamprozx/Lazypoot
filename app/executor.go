...
func isBootstrapBasedDistro(id string) bool {
	p, err := plugins.Get(id)
	if err != nil {
		return false
	}
	_, ok := p.(plugins.MutableRootfsDistro)
	return ok
}

func isDirectRootfsDistro(id string) bool {
	return isImageBasedDistro(id) || isBootstrapBasedDistro(id)
}
...
func ExecuteInstall(ctx context.Context, m Model) (chan tea.Msg, tea.Cmd) {
	...
	switch {
	case isImageBasedDistro(m.config.DistroID):
		go RunImageInstallPipeline(ctx, m.config, ch)
	case isBootstrapBasedDistro(m.config.DistroID):
		go RunDebootstrapPipeline(ctx, m.config, ch)
	default:
