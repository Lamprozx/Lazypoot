...
func RunImageInstallPipeline(ctx context.Context, cfg types.InstallConfig, ch chan tea.Msg) {
	...
	// Step 1-2: Download + verify + extract
	sendPct(ch, 5)
	...
	// Handle archives with a single top-level directory prefix (e.g. kali-arm64/)
	...
	// Setup user accounts (same logic as regular install)
	var cmds []string
	...
	// Image-based distros: keep default prompt, don't override PS1
	var filtered []string
	for _, c := range cmds {
