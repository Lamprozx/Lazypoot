...
func TestParrotVariantInfo(t *testing.T) {
	p, err := plugins.Get("parrot")
	if err != nil {
		t.Fatal("parrot plugin not found:", err)
	}
	bp, ok := p.(plugins.MutableRootfsDistro)
	if !ok {
		t.Fatal("parrot does not implement MutableRootfsDistro")
	}
	variants, err := bp.BootstrapVariants("arm64")
	if err != nil {
		t.Fatal("BootstrapVariants failed:", err)
	}
	if len(variants) != 3 {
	...
	tests := []struct {
		...
		{"core", "Core (Minimum)", []string{"parrot-core"}, 3},
		{"home", "Home Edition", []string{"parrot-interface", "parrot-interface-home"}, 7},
		{"full", "Full (Security Tools)", []string{"parrot-tools-full"}, 10},
	}
	for _, tt := range tests {
		found := false
		for _, v := range variants {
			if v.ID == tt.id {
				found = true
				if v.Label != tt.label {
					t.Errorf("%s: label=%q want %q", tt.id, v.Label, tt.label)
				}
				if len(v.PackageSets) != len(tt.sets) {
					t.Errorf("%s: %d package sets, want %d", tt.id, len(v.PackageSets), len(tt.sets))
				} else {
					for i := range v.PackageSets {
						if v.PackageSets[i] != tt.sets[i] {
							t.Errorf("%s: PackageSets[%d]=%q want %q", tt.id, i, v.PackageSets[i], tt.sets[i])
						}
					}
				}
				if v.EstimatedSizeGB != tt.gb {
					t.Errorf("%s: EstimatedSizeGB=%d want %d", tt.id, v.EstimatedSizeGB, tt.gb)
				}
			}
		}
		if !found {
			t.Errorf("variant %s not found in arm64 variants", tt.id)
		}
	}
}

func TestParrotArmhfVariants(t *testing.T) {
	p, err := plugins.Get("parrot")
	if err != nil {
		t.Fatal("parrot plugin not found:", err)
	}
	...
	variants, err := bp.BootstrapVariants("armhf")
	if err != nil {
		t.Fatal("BootstrapVariants(armhf) failed:", err)
	}
	if len(variants) != 3 {
		t.Fatalf("expected 3 variants for armhf, got %d", len(variants))
	}
}

func TestParrotUnsupportedArch(t *testing.T) {
	p, err := plugins.Get("parrot")
	if err != nil {
		t.Fatal("parrot plugin not found:", err)
	}
	...
	_, err = bp.BootstrapVariants("x86_64")
	if err == nil {
		t.Fatal("expected error for unsupported arch x86_64")
	}
}

func TestIsBootstrapBasedDistro(t *testing.T) {
	if !isBootstrapBasedDistro("parrot") {
		t.Error("isBootstrapBasedDistro('parrot') should be true")
	}
	if isBootstrapBasedDistro("kali") {
		t.Error("isBootstrapBasedDistro('kali') should be false (it's ImageBased)")
	}
	if isBootstrapBasedDistro("nonexistent") {
		t.Error("isBootstrapBasedDistro('nonexistent') should be false")
	}
}

func TestIsDirectRootfsDistro(t *testing.T) {
	if !isDirectRootfsDistro("parrot") {
		t.Error("isDirectRootfsDistro('parrot') should be true")
	}
	if !isDirectRootfsDistro("kali") {
		t.Error("isDirectRootfsDistro('kali') should be true")
	}
	if isDirectRootfsDistro("nonexistent") {
		t.Error("isDirectRootfsDistro('nonexistent') should be false")
	}
}

func TestParrotPluginMetadata(t *testing.T) {
	p, err := plugins.Get("parrot")
	if err != nil {
		t.Fatal("parrot plugin not found:", err)
	}
	if p.ID() != "parrot" {
		t.Errorf("ID()=%q want 'parrot'", p.ID())
	}
	if p.Name() != "Parrot OS" {
		t.Errorf("Name()=%q want 'Parrot OS'", p.Name())
	}
	if p.PackageManager() != "apt" {
		t.Errorf("PackageManager()=%q want 'apt'", p.PackageManager())
	}
}
